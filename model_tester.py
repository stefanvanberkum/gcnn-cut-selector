...
def load_batch_flat(sample_files, feats_type, augment_feats, normalize_feats):
    ...
    for i, filename in enumerate(sample_files):
        cand_states, cand_scores, cand_choice = utilities.load_flat_samples(filename, feats_type, 'scores',
...
def padding(output, n_vars_per_sample, fill=-1e8):
    ...
    output = tf.concat(
        [tf.pad(x, paddings=[[0, 0], [0, n_vars_max - tf.shape(x)[1]]], mode='CONSTANT', constant_values=fill) for x in
         output], axis=0)
    ...
    for batch in dataloader:
        ...
        if policy['type'] == 'gcnn':
            ...
            pred_scores = policy['model'](
                (c, ei, ev, v, tf.reduce_sum(n_cs, keepdims=True), tf.reduce_sum(n_vs, keepdims=True)),
        ...
        assert all(
            true_bestscore.numpy() == np.take_along_axis(true_scores.numpy(), best_cands.numpy().reshape((-1, 1)),
                                                         axis=1))
...
if __name__ == '__main__':
    ...
    evaluated_policies = [['gcnn', model] for model in gcnn_models] + [['ml-competitor', model] for model in
                                                                       other_models]

    fieldnames = ['policy', 'seed', ] + [f'acc@{k}' for k in top_k]
        ...
        for policy_type, policy_name in evaluated_policies:
            ...
            for seed in seeds:
                ...
                if policy['type'] == 'gcnn':
                    ...
                    policy['model'].call = tfe.defun(policy['model'].call,
                                                     input_signature=policy['model'].input_signature)
                    policy['batch_datatypes'] = [tf.float32, tf.int32, tf.float32, tf.float32, tf.int32, tf.int32,
                                                 tf.int32, tf.int32, tf.int32, tf.float32]
                    policy['batch_fun'] = load_batch_gcnn
                else:
                    # load feature normalization parameters
                    try:
                        with open(f"trained_models/{args.problem}/{policy['name']}/{seed}/normalization.pkl",
                                  'rb') as f:
                    ...
                    policy['batch_fun'] = lambda x: load_batch_flat(x, feat_specs['type'], feat_specs['augment'],
                                                                    feat_specs['qbnorm'])
                ...
                writer.writerow({**{'policy': f"{policy['type']}:{policy['name']}", 'seed': seed, },
